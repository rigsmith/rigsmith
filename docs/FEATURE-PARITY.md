...
| `status` | ✅ | ✅ | | | `--verbose`, `--since` (changed-without-changeset guard + narrowing, commit-sourced releases included), `--output` JSON plan, pre-mode reflection, no-changesets → non-zero exit. (net groups under bump headers — cosmetic.) | |
...
| `doctor` | ➖ | ✅ | 🟢 | rigsmith-only — net had no health check. | changerig: git / repo / `.changeset/config.json` (scaffolded on request) / workspace / pending changesets. shiprig adds a **release** group: `gh` auth + the publish tool each detected ecosystem needs (`dotnet`/`npm`/`cargo`; Go via tags). Shared `core/doctor` model + `core/doctorui` with `rig`/`clauderig`. | |
...
| `deps` / `dependencies` | ✅ | ✅ | ✅ | 🟢 | rig-only (no .NET-rig equivalent): a **full** dependency report — every top-level package with current + latest + whether an update is available, including the up-to-date ones the `outdated` reports drop. Composes the list report with the outdated report per ecosystem: `go list -m -u -json all` (go), `dotnet list package --format json` merged with `--outdated` (.NET), `<pm> ls --json` merged with the outdated report (node: npm/pnpm/bun/yarn-classic). `-u`/`--updates-only` filters to updates; `--vulnerable` overlays a CVE-severity column (`dotnet list package --vulnerable` for .NET; `npm`/`pnpm`/`bun`/`yarn-classic audit` for node — go/cargo need an external scanner so the column is skipped with a note); `--json` for scripts. cargo and yarn-berry have no machine-readable latest, so they fall back to the plain `outdated` list. | |
...
| `doctor` | ✅ | ✅ | ✅ | 🟢 | Per-ecosystem env checklist; non-zero exit on errors. Adds a **Tools** group (optional `extTool`s — cargo-llvm-cov/outdated/watch, wgo, dnx, reportgenerator — present/absent for the detected ecosystems) and a **Config** group (validates `.rig.json` paths resolve: `solution` file, `defaultProject` names a real project, custom-command `cwd`). The .NET check standardizes on .NET 10 — older SDKs warn (dnx features need 10), a missing SDK offers the install page on a TTY. | |
