...
| `add` / `uninstall` / `outdated` | ✅ | ✅ | ✅ | | | Per-ecosystem native; aliases `remove`/`rm`/`od`. `outdated`: dotnet/node/go native, cargo via the `cargo-outdated` subcommand. Go has no `uninstall` mapping (edit go.mod + `go mod tidy`). | |
