...
| 6b.5a. Shared manual-sync coverage checkpoint | In review (current PR) | Seal exact candidate batches before capture; acknowledge complete covered batches while preserving later arrivals, partial coverage and recovery work. [Contract](CLAUDERIG-V2-QUEUE.md#manual-sync-coverage-checkpoint-milestone-6b5a). |
| 6b.5b. Claude manual-sync coverage evidence | Next | Identify covered requests despite throttled/skipped/pruned sources, validate provenance and confirm publication before queue acknowledgement. |
