...
| 6b.6b.2c. OS restart fencing and validation | In review (current PR) | Persistent intent before process creation; verified cleanup clears it, while owner/supervisor death or uncertain cleanup blocks replacement writers. [Contract](CLAUDERIG-V2-PROCESS-LIFECYCLE.md#persistent-restart-fence-6b6b2c). |
| 6b.6b.2d. Fenced-store recovery and canonical Git supervision | Next | Prove old writers stopped before recovery; adapt canonical capture/publication/pull/merge Git subprocesses before their service boundaries accept supervised contexts. Both gates are required before queued hooks. |
