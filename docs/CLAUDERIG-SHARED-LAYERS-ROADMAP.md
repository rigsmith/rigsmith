...
| 6b.7b.2a. Writer-owned workspace cleanup | In review (current PR) | Shared staging/capture/seed/commit ownership, bounded full inventory and confined removal of reserved capture-build, publication, startup and confirmation workspaces. Claude validates binding and source separation. Sealed artifacts, recovery stores and relocated OS-temp scratch remain protected. |
| 6b.7b.2b. Referenced sealed artifacts | Next | Prove which captures/seeds/commits are disposable against pending queue work and recovery dependencies. Preserve saved artifacts from uncertain phase acknowledgements and shared seed dependencies. |
