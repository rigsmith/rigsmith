...
| 6b.6b.2d.2. Canonical workflow supervision | In review (current PR) | Active staging-lease binding, persistent operation failure checks, protected file/attribute/journal writes, and error-bearing ignore probes. Native supervised coverage/clone/reconciliation and supervisor-loss tests. Interactive tools remain refused. [Contract](CLAUDERIG-V2-PROCESS-LIFECYCLE.md#canonical-workflow-supervision-6b6b2d2). |
| 6b.6b.2d.3. Fenced-store recovery | Next | Prove old writers stopped before clearing an unconfirmed fence. Still required before queued hooks; no reset/unfence command is exposed. |
