...
| 6b.7b.2b. Queue-aware archive reclamation | In review (current PR) | Queue worker/transaction ownership and durable reflush precede cleanup. Any unfinished work protects all archives; an idle queue with completed history can reclaim verified commits/captures/seeds only without recovery/unknown private state. Queue-parent confirmation scratch is included. Selective pruning while work remains is deferred; opt-in queued Claude rollout follows these checks. |
