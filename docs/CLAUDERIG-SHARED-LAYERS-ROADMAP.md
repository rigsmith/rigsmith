...
| 6b.6a. Worker loop and controlled shutdown | In review (current PR) | Exclusive runner, idle/backoff scheduling, graceful stop, explicit incomplete drains and saved-phase restart. [Contract](CLAUDERIG-V2-QUEUE.md#worker-loop-and-controlled-shutdown-milestone-6b6a). |
| 6b.6b. Platform worker supervision | Next | Parent-death child cleanup, initial shared-history validation, startup/restart and process ownership before exposing a worker command. |
