...
**Current position (after #409):** stages 1–7 and 8a are merged. Config codec,
...
Provider/MCP rules merged in [#400](https://github.com/rigsmith/rigsmith/pull/400).
Layer composition, permission selection and source rechecks merged in
[#403](https://github.com/rigsmith/rigsmith/pull/403). Network action definitions/references merged
in [#406](https://github.com/rigsmith/rigsmith/pull/406). Selected hook header/source declarations
merged in [#407](https://github.com/rigsmith/rigsmith/pull/407). Matcher declarations/glob syntax
merged in [#408](https://github.com/rigsmith/rigsmith/pull/408). Network domain declarations
merged in [#409](https://github.com/rigsmith/rigsmith/pull/409). Current 8b.6b work
checks selected network endpoint declarations. Production layer discovery/identity checks, full permission compilation and local readiness remain; 8b.6 is not complete. Stage 8b remains
...
| 7a. Persisted Claude queue runtime | Merged: [#379](https://github.com/rigsmith/rigsmith/pull/379) | Fixed private stores, unique lifecycle queue binding, durable producer identity before enqueue, fresh resolver validation and saved-phase restart even after staging-marker loss. [Contract](CLAUDERIG-V2-QUEUE-RUNTIME.md). |
| 7b. Explicit queued Claude commands | Merged: [#380](https://github.com/rigsmith/rigsmith/pull/380) | Init, saved producer requests, enqueue, supervised foreground worker, status/retry and drain using existing Git credentials and GitHub/GitLab privacy checks (`gh`/`glab` or provider token). [Contract](CLAUDERIG-V2-QUEUE-COMMANDS.md). |
...
| 7c.2b.1. Hook-request preparation | Merged: [#384](https://github.com/rigsmith/rigsmith/pull/384) | Bounded Stop/SessionEnd payloads, source/session checks, direct-parent capture and saved producer identity; no hook installation. |
| 7c.2b.2a. Queued worker capture/flush policy | Merged: [#385](https://github.com/rigsmith/rigsmith/pull/385) | Complete requested sessions/subagents, honor all-flush, retain unrelated plain-transcript throttling and replay saved artifacts unchanged. |
...
| 8b.6. Supported-version validation | In progress | 8b.6a merged in [#397](https://github.com/rigsmith/rigsmith/pull/397): pinned schema, independent profile overlays and versioned preparation. Provider/MCP rules merged in [#400](https://github.com/rigsmith/rigsmith/pull/400). Layer composition, permission selection and source rechecks merged in [#403](https://github.com/rigsmith/rigsmith/pull/403). Network action definitions/references merged in [#406](https://github.com/rigsmith/rigsmith/pull/406). Selected hook header/source declarations merged in [#407](https://github.com/rigsmith/rigsmith/pull/407). Inherited matcher declarations/glob syntax merged in [#408](https://github.com/rigsmith/rigsmith/pull/408). Inherited network domain declarations merged in [#409](https://github.com/rigsmith/rigsmith/pull/409). Current 8b.6b: selected network endpoint declarations. Remaining: production layer discovery/identity checks, permission compilation/other requirements and local readiness. Structural validation does not prove startup readiness. [Contract](CODEXRIG-V2-CONFIG-VALIDATION.md). |
...
now have separate rows, so partial completion is visible. Canonical recovery is split into staged merge completion (6b.4a), capture integration (6b.4b.1), and unresolved recovery (6b.4b.2). Queued hooks remain disabled by default.
...
backups. [#343](https://github.com/rigsmith/rigsmith/pull/343) adds ordinary-file snapshot ordering; staged canonical merge completion merged in [#344](https://github.com/rigsmith/rigsmith/pull/344); staged completion before fresh capture merged in [#346](https://github.com/rigsmith/rigsmith/pull/346). Private unresolved-merge planning merged in [#347](https://github.com/rigsmith/rigsmith/pull/347). Recoverable file/index application merged in [#348](https://github.com/rigsmith/rigsmith/pull/348); queue recovery integration merged in [#357](https://github.com/rigsmith/rigsmith/pull/357). Queued hooks stay disabled by default.
...
Rollback restores synchronous routing through a local descriptor; the portable hook commands stay unchanged. Stop all producers, recover the inbox, drain and stop the worker, then disable routing. Disabling requires an empty inbox and idle queue and preserves recovery records and local snapshots. Because backup payloads retain their existing formats, rollback must not require restoring a backup-repository migration. Leave the default unchanged until compatibility and operational evidence justify a separate rollout decision.
