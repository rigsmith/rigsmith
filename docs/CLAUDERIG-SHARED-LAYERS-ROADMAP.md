...
| 6b.6b.2d.1. Canonical Git command-runner plumbing | In review (current PR) | Vendor-independent runner selection for buffered core Git and backup attributes; explicit streaming/interactive refusal; real supervised Git byte-preservation tests. [Contract](CLAUDERIG-V2-PROCESS-LIFECYCLE.md#canonical-git-command-runner-plumbing-6b6b2d1). Service guards remain. |
| 6b.6b.2d.2. Canonical workflow supervision and fenced-store recovery | Next | Bind active staging leases and propagate failures through canonical capture/publication/pull/merge workflows and boolean probes such as `IsIgnored` before accepting supervised contexts; prove old writers stopped before recovery. Both gates are required before queued hooks. |
