...
| 6b.6b.2a. Windows child ownership at creation | In review (current PR) | Atomically place a suspended anchor in the job and inherit ownership when starting commands. Forced owner-death tests cover startup boundaries. [Contract](CLAUDERIG-V2-PROCESS-LIFECYCLE.md). |
| 6b.6b.2b. Unix parent-death supervision | Next | Keep ownership until external helpers have stopped after worker death. |
| 6b.6b.2c. OS restart fencing and validation | Planned | Prevent a replacement worker from overlapping cleanup of old helpers; validate platform startup/restart before command rollout. |
