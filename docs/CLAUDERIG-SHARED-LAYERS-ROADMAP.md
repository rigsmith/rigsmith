...
| 6b.6b.2d.3a. Linux/macOS fenced-store recovery | In review (current PR) | Durable ownership evidence, a pinned process group before command startup, and exclusive recovery after OS proof. Saved capture and queue state stay intact. [Contract](CLAUDERIG-V2-PROCESS-LIFECYCLE.md#unix-fenced-store-recovery-6b6b2d3a). |
| 6b.6b.2d.3b. Windows fenced-store recovery | Next | Positive proof of asynchronous job termination. Missing job names alone are insufficient; Windows/legacy fences stay blocked until supported evidence is available. |
