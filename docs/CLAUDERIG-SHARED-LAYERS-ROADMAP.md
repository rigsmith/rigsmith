...
| 6b.7b.1. Artifact-store capacity and interrupted archive writes | In review (current PR) | Optional sealed-byte admission limits, direct-file inventory and exclusive cleanup of interrupted archive writes. Existing artifacts remain reusable; seed substores inherit an independent limit. [Contract](CLAUDERIG-V2-CAPTURE-ARTIFACTS.md#store-capacity-and-interrupted-archive-writes-6b7b1). |
| 6b.7b.2. Referenced artifacts and build workspaces | Next | Derive live capture/seed/commit references and acquire relevant writer leases before reclaiming disposable sealed artifacts and build/publication scratch. Preserve every recovery intent. |
