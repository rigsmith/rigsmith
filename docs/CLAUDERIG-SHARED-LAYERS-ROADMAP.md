...
**Completed:** SSH transport with explicit identity/known-hosts files, isolated
OpenSSH configuration, and local SSH publication/authentication fixtures merged in
[#325](https://github.com/rigsmith/rigsmith/pull/325) as `e1aba8bd`.
**In review in [#326](https://github.com/rigsmith/rigsmith/pull/326):** an explicit HTTPS credential-helper boundary: pinned repository
and username, noninteractive controls, bounded lookup, strict credential parsing,
expiry checks and owned helper cleanup. This is internal composition support, not
automatic discovery or a claim that every OS keychain is safe unattended.
SSH-agent support, platform helper selection and noninteractive keychain fixtures,
native conflict recovery, parent-death recovery, queued hooks and CodexRig remain planned.
