...
| 8b. Codex config portability | In progress | TOML codec (8b.1) merged in [#390](https://github.com/rigsmith/rigsmith/pull/390). Bounded config file capture (8b.2) merged in [#391](https://github.com/rigsmith/rigsmith/pull/391). Current: machine-local TOML path policy (8b.3). Remaining: guarded restore/usable-config validation, hooks/customizations, independent state/repository and sync/restore wiring. [Path policy](CODEXRIG-V2-CONFIG-PATHS.md). |
