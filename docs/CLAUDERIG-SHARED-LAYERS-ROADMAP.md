...
| 6b.5b. Claude manual-sync coverage evidence | In review (current PR) | `SyncWithCoverage` requires fresh session/subagent bytes, matching provenance, retained files and fresh remote confirmation before acknowledging complete batches. Command/hook wiring stays in rollout. [Contract](CLAUDERIG-V2-QUEUE.md#claude-manual-sync-evidence-milestone-6b5b). |
| 6b.6. Worker lifecycle | Next | Parent-death recovery, startup/restart, stop and draining. |
