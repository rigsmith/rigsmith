...
| 6b.6b.2b. Unix parent-death supervision | In review (current PR) | Explicit per-command supervisor in a separate session, inherited staging lease, queued phase/retry integration and worker-death tests before/after command startup. [Contract](CLAUDERIG-V2-PROCESS-LIFECYCLE.md#unix-worker-death-supervision-6b6b2b). |
| 6b.6b.2c. OS restart fencing and validation | Next | Fence supervisor failure/uncertain cleanup and asynchronous Windows termination; prevent a replacement worker from overlapping cleanup of old helpers; validate platform startup/restart before command rollout. |
