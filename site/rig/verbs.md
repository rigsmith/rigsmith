...
| `stack rm <repo>` | Remove a repo — manifest entry, directory and overlay redirects; refuses while it holds unsent work or files git ignores (`--force`), `--keep-tree` keeps the directory |
| `stack seed <dir>` | Export the root files as a small repo; `stack init` on a clone of it rebuilds every member at its recorded cursor, or from the fork branch it was last proposed to; refuses while a member holds unsent commits (`--force`) |
