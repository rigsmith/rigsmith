...
`relrig` are a faithful port of that model — pinned against a golden corpus that
includes live @changesets output — with a few deliberate differences.

| | @changesets/cli | changerig / relrig |
|---|---|---|
...
| Release orchestration | GitHub Action + bespoke scripts | `relrig release` — a configurable pipeline (`.changeset/release.jsonc`) |
...
**Use changerig / relrig** if your repo is polyglot (or just isn't Node), if you
want a single binary with no runtime to install, or if you want the release
*orchestration* (`relrig release`) as a first-class, config-driven pipeline
