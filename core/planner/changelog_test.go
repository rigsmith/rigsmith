...
func TestRenderGroupsByTypeAndLeadsWithScope(t *testing.T) {
	...
	got := renderSections("1.1.0", "", false, changes, config.DefaultChangelogGroups, nil)
...
func TestScopeOrderIsConfigurable(t *testing.T) {
	...
	got := renderSections("1.1.0", "", false, changes, config.DefaultChangelogGroups, []string{"rig", "clauderig"})
...
func TestTrailingNewlinesDoNotBecomeBlankContinuations(t *testing.T) {
	got := renderSections("1.0.1", "", false, []plugin.ChangelogChange{
...
func TestRenderSectionsFoldsUntypedChangesIntoTypedSections(t *testing.T) {
	got := renderSections("2.0.0", "", false, []plugin.ChangelogChange{
...
func TestRenderSectionsKeepsBumpHeadingsWhenUntypedOrUngrouped(t *testing.T) {
	untyped := renderSections("1.1.0", "", false, []plugin.ChangelogChange{
	...
	ungrouped := renderSections("1.1.0", "", false, []plugin.ChangelogChange{
...
func TestRenderSectionsIgnoresEmptyTypedAndNoneChanges(t *testing.T) {
	...
	typed := renderSections("1.1.0", "", false, []plugin.ChangelogChange{
...
func TestRenderSectionsGivesTypedDependenciesTheirOwnSection(t *testing.T) {
	...
	untyped := renderSections("1.0.1", "", false, []plugin.ChangelogChange{{Bump: "patch", Summary: "A fix"}, deps}, config.DefaultChangelogGroups, nil)
...
func TestRenderSectionsKeepsAGroupNamedLikeTheDependencies(t *testing.T) {
	...
	got := renderSections("1.1.0", "", false, []plugin.ChangelogChange{
		{Bump: "minor", Type: "feat", Summary: "A feature"},
...
func TestRenderSectionsCapsBumpHeadingsAtTheReleaseBump(t *testing.T) {
	untyped := renderSections("0.4.0", "minor", false, []plugin.ChangelogChange{
	...
	typed := renderSections("0.4.0", "minor", false, []plugin.ChangelogChange{
		{Bump: "major", Summary: "Break it"},
...
func TestRenderSectionsRaisesTheDecidingChangesToTheReleaseBump(t *testing.T) {
	got := renderSections("2.0.0", "major", true, []plugin.ChangelogChange{
...
// A release raised by a group's coordination, not by hand, keeps each
// change under its own bump, as @changesets does for a linked or fixed
// member: pkg-b's fix under Patch Changes in a 1.1.0 release.
func TestRenderSectionsDoesNotRaiseForAGroupsCoordinatedBump(t *testing.T) {
	got := renderSections("1.1.0", "minor", false, []plugin.ChangelogChange{
		{Bump: "patch", Summary: "A fix"},
	}, config.DefaultChangelogGroups, nil)
	if want := "## 1.1.0\n\n### Patch Changes\n\n- A fix\n"; got != want {
		t.Errorf("got:\n%s\nwant:\n%s", got, want)
