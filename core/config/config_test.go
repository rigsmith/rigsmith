...
func TestPrivatePackagesDefaultToIgnored(t *testing.T) {
	cases := []struct {
		name     string
		json     string
		versions bool
	}{
		{"unset (v3 default)", `{}`, false},
		{"bare false", `{ "privatePackages": false }`, false},
		{"version false", `{ "privatePackages": { "version": false, "tag": false } }`, false},
		{"version true", `{ "privatePackages": { "version": true, "tag": false } }`, true},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := cfg.VersionsPrivate(); got != tc.versions {
				t.Fatalf("VersionsPrivate = %v, want %v", got, tc.versions)
			}
			cfg.MarkPrivate([]string{"secret"})
			if got := cfg.IsIgnored("secret"); got == tc.versions {
				t.Errorf("IsIgnored(private) = %v, want %v", got, !tc.versions)
			}
			if cfg.IsIgnored("public") {
				t.Error("a package not marked private must not be ignored")
			}
		})
	}
}
