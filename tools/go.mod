// Module tools holds dev-tooling tool directives only — it ships no code and is
// not imported by any other module. Its sole purpose is to pin tools (lefthook)
// so the git hooks resolve them via `go tool` without relying on $PATH. Keeping
// them here keeps their dependency trees out of cli/core/etc.
module github.com/rigsmith/tools
