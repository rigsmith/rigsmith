...
| `account` | `add` / `list` / `run` / `prepare` / `switch` / `sessions` / `alias` / `map` / `disable` / `remove` / `doctor` — several Claude Code logins on one machine. `run` launches `claude` as an account in an isolated, self-refreshing profile; `prepare` readies that same profile and prints its `CLAUDE_CONFIG_DIR` for a program that launches `claude` itself (bare stdout is the dir; `--json` is one object with a stable `reason` on refusal); `switch` swaps the machine-wide login and is guarded against live sessions (alias: `acct`) |
