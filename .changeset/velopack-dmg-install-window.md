...
Also fixes the layout silently reverting to a default icon grid: the Finder-scripting step now targets the volume by the name it actually mounted under (a prior copy of the dmg still open no longer makes it script the wrong window), brings Finder to the front so icon positions commit, and unmounts cleanly after the positions settle so they survive into the compressed image.
