...
The window grows a **Repository** panel carrying the same numbers, with the ratio flagged amber once history costs four times the content, and a prune dialog offering a week, a month or three, and a Repack button beside it.

It also answers the question the panel exists for — how far back can I go — as a span rather than a date. `retained 28 hours` is read at a glance; `2026-08-27 08:18` makes the reader do arithmetic, and after a squash the answer is usually "less than you think", which is exactly when nobody does it. It will not fold closer than three days: the activity feed's file lists are read out of recent commits, and a prune that left one commit standing would take "what did that sync actually do" away with it.
