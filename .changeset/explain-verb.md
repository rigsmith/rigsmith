...
- **Config parse warnings are surfaced at all.** A malformed file degraded to defaults, an unknown top-level key, a script file that wouldn't load — these were collected into `Config.Warnings` and then read by nobody, so a broken config looked like an accepted one. They print on the same channel now. Completion requests are exempt: a shell parses that output.
