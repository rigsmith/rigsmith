...
Detection and removal are held to agreeing on every PEM shape, in both directions, because a disagreement either way is a defect: detected-but-unremovable refuses the sync for ever, and removable-but-undetected is prose rewritten on a guess. The grammar both sides use — separator, optional attribute lines, then twenty base64 characters — is now written once and shared, so they cannot drift apart again.
