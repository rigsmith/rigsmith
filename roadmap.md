...
| Unresolved merge recovery: private audited plan | In review: [#347](https://github.com/rigsmith/rigsmith/pull/347). Recreate supported conflicts privately, validate index provenance, and export an audited candidate without changing staging. |
| Unresolved merge recovery: safe application and restart | Next. Preserve later edits, install the plan safely, and recover interrupted application. |
| Unresolved merge recovery: queue integration | Planned after safe application. Use recovery before fresh capture and retained publication. |
