...
changerig version --only pkg                # version only this release group; the rest wait for a later run
changerig version --yes --release-as pkg=2.0.0   # release a package at an exact version (CI)
...
# your app and its forked libraries fused into one history, each still leaving as itself (private repos: just `gh auth login`):
...
rig stack status / pull [repo]              # what upstream did, where each topic's PR is, and taking it
...
- [docs/GITHUB-ACTIONS.md](docs/GITHUB-ACTIONS.md) — the two composite Actions: `release` (the version-PR/publish loop) and `require-changeset` (the per-PR gate — comment + block, no hosted bot). **Built** (`.github/actions/`). For releasing, [rigsmith/shiprig-action](https://github.com/rigsmith/shiprig-action) is the recommended successor, and what rigsmith itself uses.
