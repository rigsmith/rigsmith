...
| [`cmd/rig/`](cmd/rig/) | `rig` | The convention-first dev launcher (run/build/test/format across .NET, Node, Go, Rust), plus parallel worktrees and **stackspaces** (`rig stack`) — your app and the repos you have forked, fused into one history. Export a portable seed with `rig stack seed ../rigstack-acme`. Successor to the .NET/Node [`rig`](https://github.com/JohnCampionJr/rig). |
...
| [`cmd/clauderig/`](cmd/clauderig/) | `clauderig` | Sync your Claude Code setup (config, skills, session history) across machines via a private git repo, with cross-OS path correction, complete staged-text secret scanning, Git byte preservation, transcript chunking (on for new configurations), and coordinated local backup operations. V2 adds an [explicit foreground queue workflow](docs/CLAUDERIG-V2-QUEUE-COMMANDS.md) with saved manual/hook requests, managed hook admission and recovery, optional local hook routing with guarded rollback, manual sync acknowledgement, resumable publication, and complete requested-session capture with normal throttling for unrelated plain transcripts. Restore checks that recreated memory links resolve inside the selected folder and skips links pointing outside it. See [docs/CLAUDERIG-DESIGN.md](docs/CLAUDERIG-DESIGN.md). |
| [`cmd/codexrig/`](cmd/codexrig/) | `codexrig` | V2 source preview: read-only inventory of Codex configuration and customization candidates. Separate executable and adapter; sync, restore and queued hooks are still planned. |
