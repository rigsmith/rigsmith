...
// Launcher templates. Placeholders ({{NAME}}, {{REPO}}, {{MOD}}, {{DEV}}) are
// substituted by writeLauncher; on Windows it also rewrites \n to \r\n. Using
// raw literals keeps the shell/batch readable (no %%-escaping) and the `%` in
// the .cmd files literal.
const (
	devWrapperSh = `#!/bin/sh
...
if "%~1"=="" (echo usage: {{NAME}}-wt ^<branch^|worktree-path^> [args...]>&2& exit /b 2)
set "TARGET=%~1"
set "SRC="
