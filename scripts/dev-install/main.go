...
# repo. Per-source builds get separate dirs. (go run -C would reset cwd to the
# repo, resolving the wrong workspace.)
set -e
...
if not defined SRC if exist "{{ROUTE}}" set /p SRC=<"{{ROUTE}}"
if defined SRC if not exist "%SRC%\" (
	echo {{NAME}}-dev: pinned route %SRC% is gone — using repo>&2
	set "SRC="
)
