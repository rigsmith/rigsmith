...
// binary once from the repo root (located via go.mod), every test runs the
...
func TestMain(m *testing.M) {
	...
	changerigBin, err = buildBinary(root, tmp, "changerig", "./cmd/changerig")
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.RemoveAll(tmp)
		os.Exit(1)
	}
	shiprigBin, err = buildBinary(root, tmp, "shiprig", "./cmd/shiprig")
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.RemoveAll(tmp)
		os.Exit(1)
