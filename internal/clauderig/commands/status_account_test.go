...
func TestPrintAccountLine(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			in:   status.AccountInfo{Email: "john@work.com", Subscription: "max", InSync: true},
		...
		},
		{
			...
			in:   status.AccountInfo{Email: "john@work.com", Alias: "dev", InSync: true},
		...
		},
		{
			...
			in:   status.AccountInfo{Email: "john@work.com", InSync: false},
			want: []string{"desynced", "account doctor"},
		},
		{
			name: "a login clauderig has never captured says so",
			in:   status.AccountInfo{Email: "john@new.com", InSync: true, Untracked: true},
		...
		},
		{
			...
			in:   status.AccountInfo{Email: "john@work.com", InSync: true, PointerEmail: "john@home.com"},
			want: []string{"pointer", "john@home.com"},
		},
		{
			name: "no login at all",
			in:   status.AccountInfo{},
