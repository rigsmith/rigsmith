...
func TestSearchSessions_DedupTitleAndResume(t *testing.T) {
	...
	if err := searchSessions(&out, &errw, testMachine(t.TempDir()), targets, roots, "billing", sessionScope{}); err != nil {
		t.Fatal(err)
	}
...
func TestSearchSessions_TitleOnly(t *testing.T) {
	...
	err := searchSessions(&out, &errw, testMachine(t.TempDir()),
		[]search.Target{{Label: "cli", Dir: live}}, []session.Root{{Label: "desktop", Base: desk}}, "rocket", sessionScope{})
	if err != nil {
		t.Fatal(err)
	}
	got := stripANSI(out.String())
...
func TestSearchSessions_RepoOnlyNotResumable(t *testing.T) {
	...
	var out, errw bytes.Buffer
	err := searchSessions(&out, &errw, testMachine(t.TempDir()),
		[]search.Target{{Label: "cli", Dir: live}, {Label: "repo", Dir: repo}}, nil, "widget", sessionScope{})
	if err != nil {
		t.Fatal(err)
	}
	got := stripANSI(out.String())
...
func TestSearchSessions_TitleOnlyButLiveTranscriptResumable(t *testing.T) {
	...
	var out, errw bytes.Buffer
	err := searchSessions(&out, &errw, testMachine(t.TempDir()),
		[]search.Target{{Label: "cli", Dir: live}}, []session.Root{{Label: "desktop", Base: desk}}, "kubernetes", sessionScope{})
	if err != nil {
		t.Fatal(err)
	}
	got := stripANSI(out.String())
