...
func TestQueueHookRefusesInputBeforeIdentityOrWrite(t *testing.T) {
	...
	for name, data := range map[string]string{
		...
		"null-agent":   strings.Replace(valid, `{`, `{"agent_id":null,`, 1),
		"number-agent": strings.Replace(valid, `{`, `{"agent_id":1,`, 1),
		"bool-agent":   strings.Replace(valid, `{`, `{"agent_id":false,`, 1),
		"object-agent": strings.Replace(valid, `{`, `{"agent_id":{},`, 1),
		"array-agent":  strings.Replace(valid, `{`, `{"agent_id":[],`, 1),
