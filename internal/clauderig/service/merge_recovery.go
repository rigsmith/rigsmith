...
func artifactMergePolicy(p commitartifact.MergeFinishPolicy) commitartifact.MergeStagePolicy {
	p.Validate, p.Audit = backupgit.ValidateTree, engine.CheckPublishContext
	return commitartifact.MergeStagePolicy{PolicyID: "claude-merge-recovery-v1", MergePlanPolicy: commitartifact.MergePlanPolicy{
...
func recoverArtifactMerge(ctx context.Context, stage string, store commitartifact.MergeStageStore, p commitartifact.MergeFinishPolicy) (string, error) {
	policy := artifactMergePolicy(p)
	saved, err := store.HasIntent(ctx, stage, policy)
	if err != nil {
		return "", err
	}
	if saved {
