...
`), 0o644); err != nil {
  ...
  "gone": {"upstream": "github.com/acme/gone", "fork": "github.com/you/gone"}
	...
	t.Run("an overlay already rewritten before a later ecosystem failed is put back too", func(t *testing.T) {
		// .NET is asked before Go. Its generated overlay is rewritten fine;
		// then a go.work somebody wrote by hand stops the Go rewrite — and
		// the .NET overlay must not stay describing a stackspace without
		// the member the manifest still has.
		root := t.TempDir()
		mustGitStack(t, root, "init", "-q", "-b", "main")
		mustGitStack(t, root, "config", "user.email", "t@t")
		mustGitStack(t, root, "config", "user.name", "t")
  ...
  "gone": {"upstream": "github.com/acme/gone", "fork": "github.com/you/gone"}
		...
		for rel, body := range map[string]string{
			"lib/go.mod":              "module example.com/acme/lib\n\ngo 1.24\n",
			"app/go.mod":              "module example.com/you/app\n\ngo 1.24\n\nrequire example.com/acme/lib v0.0.0\n",
			"go.work":                 "go 1.24\n\nuse ./app\n", // hand-written, and missing ./lib
			"Directory.Build.targets": "<!-- generated by rig: package references redirected to sources in this tree -->\n<Project/>\n",
		} {
		...
		out, err := runRm(t, "gone")
		if err == nil || !strings.Contains(err.Error(), "go.work") || !strings.Contains(err.Error(), "overlay files already rewritten") {
			t.Fatalf("err = %v\n%s", err, out)
		}
		got, rerr := os.ReadFile(filepath.Join(root, "Directory.Build.targets"))
		if rerr != nil || !strings.Contains(string(got), "<Project/>") {
			t.Errorf("the .NET overlay was left rewritten:\n%s", got)
		}
		if st := strings.TrimSpace(mustGitStack(t, root, "status", "--porcelain")); st != "" {
			t.Errorf("failed rm left the tree dirty:\n%s", st)
		}
